"""
title: PubMed Deep Research Tool v3
author: Beau D'Amore www.damore.ai
version: 3.0.0
description: Deep PubMed research with native OpenWebUI integrations and PMC figure image download/storage.
...
class Tools:
    class Valves(BaseModel):
        ...
        reranker_results: int = Field(
            default=0,
            description="Number of results to return to the LLM after reranking (0 disables reranking and returns max_results). All max_results articles are still fetched and archived to the knowledge base, but only this many are included in the LLM response to stay within context limits.",
        )
...
- LLM Output Limit: {self.valves.reranker_results if self.valves.reranker_results > 0 else self.valves.max_results}
...
"""
        ...
        try:
            ...
            if documents and documents[0]:
                ...
                # Extract PMIDs from RAG results for deduplication only
                text_pmids = set()
            ...
            all_pmid_keys = list(articles_by_pmid.keys())
            report_pmids = all_pmid_keys[:output_limit]

            report_lines = [
                f"Search Query: {query}",
                f"Retrieved At (UTC): {timestamp}",
                f"Total Articles Found: {len(articles)}",
                f"Articles in Report: {len(report_pmids)} (of {len(all_pmid_keys)})",
                "",
            ]

            for index, pmid in enumerate(report_pmids, start=1):
                article_lines = []
                    ...
                    try:
                        ...
                        if self.valves.enable_figure_download:
                            ...
                            if article_pmcid and article_figures:
                                ...
                                # Collect graphic href basenames for targeted extraction
                                fig_hrefs = []
            ...
            # Report is already limited to output_limit articles to respect context window.
            # All articles are archived to KB for future RAG retrieval regardless.
            omitted_note = ""
            if len(all_pmid_keys) > output_limit:
                omitted_note = (
                    f"\n\n> **Note**: {len(all_pmid_keys) - output_limit} additional article(s) "
                    f"were archived to the knowledge base but omitted from this response "
                    f"to stay within context limits (reranker_results={output_limit})."
                )
...
class KnowledgeRepository:
    ...
    def download_pmc_figures(
        ...
        PMC no longer serves individual figure images at direct URLs. Instead,
        this fetches the OA package tarball from NCBI FTP and extracts image
        files from it.

        Args:
            pmcid: PMC ID (e.g. "PMC13025705")
            figure_hrefs: Optional list of graphic href values from fetch_pmc_figures
                to match against tarball contents. If None, extracts all images.
            max_figures: Max images to extract (0 = unlimited)

        Returns:
            List of dicts with keys: filename, data (bytes), content_type
        """
        if not pmcid:
            return []

        # Step 1: Query OA service for tarball URL
        ...
        try:
            with tarfile.open(fileobj=io.BytesIO(resp.content), mode="r:gz") as tar:
                for member in tar.getmembers():
                    ...
                    # If we have specific hrefs, filter to matching images
                    if href_stems:
                        stem = basename.rsplit(".", 1)[0].lower()
        ...
        # Post-process: deduplicate — prefer JPG/PNG over GIF for same figure stem
        ...
        for i, r in enumerate(results):
            ...
            if stem in seen_stems:
                ...
                # Keep the higher-quality format (JPEG/PNG > GIF)
